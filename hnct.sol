...
contract HNCT is IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using SafeERC20 for IERC20;

    string public constant name = "HNC Token";
    string public constant symbol = "HNCT";
    ...
    TaxRates internal _taxRates = TaxRates(2, 3, 1, 2, 2, 10);
    ...
    constructor() {
        ...
        uint256 tokensToBurn = _stats.totalTokens.mul(30).div(100);
    ...
    function transferFrom(
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "HNCT: transfer amount exceeds allowance"));
        return true;
    }

    function approve(address spender, uint256 amount) external override returns (bool) {
        require((amount == 0) || (_allowances[_msgSender()][spender] == 0), "HNCT: approve from non-zero to non-zero allowance");
    ...
    function decreaseAllowance(address spender, uint256 value) external returns (bool) {
        ...
        require(oldValue >= value, "HNCT: cannot decrease allowance below zero");
    ...
    function updateLiquidityThreshold(uint256 threshold) external onlyOwner {
        require(threshold > 0, "HNCT: Cannot set threshold to zero");
    ...
    function reflectionFromToken(uint256 amountTokens, bool deductFees) public view returns (uint256) {
        require(amountTokens <= _stats.totalTokens, "HNCT: amount must be less than total supply");
	...
	function setRouterAddress(address newRouter) public onlyOwner() {
        IPancakeRouter02 _router = IPancakeRouter02(newRouter);
        pair = IPancakeFactory(_router.factory()).createPair(address(this), _router.WETH());
        router = _router;
    }
    ...
    function tokenFromReflection(uint256 reflectionAmount) internal view returns (uint256) {
        require(reflectionAmount <= _stats.totalReflection, "HNCT: amount has to be less or equal to total reflection");
    ...
    function extendedTransferStandard(
        ...
        _balances[sender].reflection = _balances[sender].reflection.sub(
            ...
            "HNCT: transfer amount exceeds balance"
        );
    ...
    function extendedTransferExcluded(
        ...
        if (isToExcluded && isFromExcluded) {
            ...
            _balances[sender].tokens = _balances[sender].tokens.sub(amount, "HNCT: transfer amount exceeds balance");
            _balances[recipient].reflection = _balances[recipient].reflection.add(params.reflectionTransferAmount);
    ...
    function _transfer(
        ...
        require(to != address(0), "HNCT: transfer to the zero address");
        require(from != address(0), "HNCT: transfer from the zero address");
        require(amount > 0, "HNCT: transfer amount must be greater than zero");
    ...
    function _approve(
        ...
        require(spender != address(0), "HNCT: approve to the zero address");
        require(owner != address(0), "HNCT: approve from the zero address");
